...
import random
import string
import extra_streamlit_components as stx  # pip install extra-streamlit-components 필요
import streamlit as st
# config.py가 같은 폴더에 있으므로 import
import config 
import pandas as pd
from pandas.tseries.offsets import BusinessDay # BusinessDay를 직접 import하여 사용
...
def send_verification_email(to_email, code):
    ...
    msg["From"] = config.MAIL_FROM  # config.py의 발신자 주소
    msg["To"] = to_email
    ...
    try:
        # config.py에 정의된 SMTP 정보 사용
        with smtplib.SMTP(config.MAIL_SMTP_HOST, config.MAIL_SMTP_PORT, timeout=10) as server:
            server.ehlo()
            server.starttls(context=context) # context를 인수로 전달
            server.ehlo()
        ...
        with col_login:
            if st.button("로그인", type="primary"):
                ...
                # 코드 일치 여부 확인
                    ...
                    # 1) 쿠키 설정 (장기 유지용)
                    cookie_manager.set("eers_auth_token", st.session_state["target_email"], expires_at=expire_date)
...
try:
    ...
    from gui_app import (
        MIN_SYNC_DATE, _get_last_sync_datetime_from_meta,
        _set_last_sync_datetime_to_meta, is_weekend, prev_business_day, _as_date,
        fmt_phone, fetch_dlvr_header, fetch_dlvr_detail
    )
    from gui_app import only_digits as only_digits_gui

except ImportError as e:
    st.error(f"필수 모듈 로드 오류: {e}. 모든 .py 파일이 같은 폴더에 있는지 확인하세요.")
    st.stop()
...
def mail_send_page():
    ...
    with col_office:
        ...
        raw = (
            config.MAIL_FROM_NAME.split()[0].replace("본부", "직할")
            if config.MAIL_FROM_NAME else None
        )
